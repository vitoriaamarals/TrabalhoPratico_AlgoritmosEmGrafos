...
private:
    ...
    int vertices;
    bool direcionado;
    vector<vector<vector<int>>> adj_list; // lista de adjacência: [vértice, peso, id_aresta]
...
public:
    ...
    Grafo(int vertices, bool direcionado)
        ...
        adj_list_transposto.resize(vertices); // Iniciar o grafo transposto
    }

    void adicionarAresta(int id_aresta, int u, int v, int peso) {
        adj_list[u].push_back({v, peso, id_aresta});
        adj_list_transposto[v].push_back(u); // Adiciona a aresta inversa no grafo transposto
        if (!direcionado) {
            adj_list[v].push_back({u, peso, id_aresta});
            adj_list_transposto[u].push_back(v); // Adiciona a aresta inversa para o grafo não direcionado
        }
    }
    ...
    void DFSArticulacao(int u, vector<bool> &visitado, vector<int> &disc, vector<int> &low, vector<int> &pai, vector<bool> &articulation_point, int &time) {
        ...
        for (const auto &edge : adj_list[u]) {
            ...
            if (!visitado[v]) {
                filho++;
                pai[v] = u;
                DFSArticulacao(v, visitado, disc, low, pai, articulation_point, time);
    ...
    void DFSPontes(int u, vector<bool> &visitado, vector<int> &disc, vector<int> &low, vector<int> &pai, vector<pair<int, int>> &pontes, int &time) {
        ...
        for (const auto &edge : adj_list[u]) {
            ...
            if (!visitado[v]) {
                ...
                // Se a condição for verdadeira, (u, v) é uma ponte
                if (low[v] > disc[u]) {
    ...
    void encontrarVerticesArticulacao() {
        ...
        vector<bool> articulation_point(vertices, false);
        int time = 0;
        ...
        vector<int> pontos_de_articulacao;
        for (int i = 0; i < vertices; i++) {
        ...
        if (count == 0 and direcionado == false){
            cout << "-1";
        }
        cout << endl;
    }
    ...
    void DFS(int v, vector<bool> &visitado, stack<int> &Stack) {
        ...
        Stack.push(v); // Empilha o vértice após visitar todos os adjacentes
    }
    ...
    int encontrarCaminhoMinimo() {
        ...
        while (!minHeap.empty()) {
            ...
            if (d > dist[u]) {
                continue; // Ignorar se já encontrarmos um caminho melhor
            }
        ...
        // Retorna a distância do vértice 0 ao vértice n-1
    ...
    int encontrarFluxoMaximo() {
        ...
        while (BFS(s, t, pai)) {
            ...
            for (int v = t; v != s; v = pai[v]) {
                ...
                for (auto& edge : adj_list[u]) {
                    if (edge[0] == v) {
                        edge[1] -= caminho_fluxo; // Subtrai o fluxo do caminho
                        break;
                    }
                }
...
void calcularGraus() {
        std::vector<int> grauEntrada(vertices, 0);
        std::vector<int> grauSaida(vertices, 0);
        ...
        std::cout << "Grau de saída:" << std::endl;
        for (int i = 0; i < vertices; ++i) {
            std::cout << "Vértice " << i << ": " << grauSaida[i] << std::endl;
        }

        std::cout << "Grau de entrada:" << std::endl;
        for (int i = 0; i < vertices; ++i) {
            std::cout << "Vértice " << i << ": " << grauEntrada[i] << std::endl;
        }
...
int main() {
    ...
    //int pseudossimetrico = grafo.verificarPseudossimetria();
